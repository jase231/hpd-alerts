...
// credit to Paul Fournel on stackoverlow: https://stackoverflow.com/a/23395136
...
function startLiveUpdates() {
    setInterval(fetchNewIncidents, 30000); // 30  secs
}
...
function fetchNewIncidents() {
    $.getJSON('/getAlerts')
        .then(data => {
            ...
            removeOldIncidents(data);
        })
        .catch(error => console.error('Error fetching incidents:', error));
}

function removeOldIncidents(newData) {
    let newIds = Object.keys(newData);
    Object.keys(markers).forEach(id => {
        if (!newIds.includes(id)) {
            markers[id].remove();
            delete markers[id];
            $(`#incident-list .item[data-id="${id}"]`).remove();
        }
    });
}

function updateIncident(incident) {
    let marker = markers[incident.ID];
    marker.setLatLng([incident.Location.Lat, incident.Location.Lng]);
    marker.setPopupContent(`<b>${incident.Type}</b><br>${incident.Block}<br>${incident.Received}`);

    let listItem = $(`#incident-list .item[data-id="${incident.ID}"]`);
    listItem.find('.header').text(incident.Type);
    listItem.find('.description').html(`
        Block: ${incident.Block}<br>
        Received: ${incident.Received}<br>
        Status: ${incident.CallStatus}<br>
        District: ${incident.Distr}
